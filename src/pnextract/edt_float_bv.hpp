...
inline void squared_edt_1d_multi_seg(pasta::BitVector const &segids, float *d,
                                     int64_t seg_start, const int64_t n,
                                     const float anisotropy,
                                     const bool black_border = false) noexcept {
  ...
  bool working_segid = segids[seg_start];

  if (black_border) {
    d[0] = working_segid ? anisotropy : 0.0f; // 0 or 1
  } else {
    d[0] = working_segid ? std::numeric_limits<float>::infinity() : 0.0f;
  }

  for (i = 1; i < n; ++i) {
    bool cur = segids[seg_start + i];
    if (!cur) {
      d[i] = 0.0f;
    } else if (cur == working_segid) {
      d[i] = d[i - 1] + anisotropy;
    } else {
      d[i] = anisotropy;
      d[i - 1] = segids[seg_start + i - 1] ? anisotropy : 0.0f;
      working_segid = segids[seg_start + i];
    }
  }

  int64_t min_bound = 0;
  if (black_border) {
    d[n - 1] = segids[seg_start + n - 1] ? anisotropy : 0.0f;
    min_bound = 1;
  }

  for (i = (n - 2); i >= min_bound; --i) {
    d[i] = std::fminf(d[i], d[i + 1] + anisotropy);
  }
 ...
 */
inline void squared_edt_1d_parabolic(float *f, const int64_t n,
                                     const int64_t stride,
                                     const float anisotropy,
                                     const bool black_border_left,
                                     const bool black_border_right) noexcept {
  ...
  for (int64_t i = 0; i < n; i++) {
    ...
    f[i * stride] = static_cast<float>(w2 * squaref(i - v[k]) + ff[v[k]]);
    ...
    if (black_border_left && black_border_right) {
      envelope = std::fmin(w2 * squaref(i + 1), w2 * squaref(n - i));
      f[i * stride] = std::fminf(static_cast<float>(envelope), f[i * stride]);
    } else if (black_border_left) {
      f[i * stride] =
          std::fminf(w2 * squaref(i + 1), static_cast<float>(f[i * stride]));
    } else if (black_border_right) {
      f[i * stride] =
          std::fminf(w2 * squaref(n - i), static_cast<float>(f[i * stride]));
    }
  }
}
...
inline void squared_edt_1d_parabolic(float *f, const int64_t n,
                                     const int64_t stride,
                                     const float anisotropy) noexcept {
  ...
  for (int64_t i = 0; i < n; i++) {
    ...
    f[i * stride] = w2 * squaref(i - v[k]) + ff[v[k]];
    // Two lines below only about 3% of perf cost, thought it would be more
    // They are unnecessary if you add a black border around the image.
    envelope = std::fmin(w2 * squaref(i + 1), w2 * squaref(n - i));
    f[i * stride] = std::fminf(static_cast<float>(envelope), f[i * stride]);
...
inline void _squared_edt_1d_parabolic(float *f, const int64_t n,
                                      const int64_t stride,
                                      const float anisotropy,
                                      const bool black_border_left,
                                      const bool black_border_right) noexcept {
...
_binary_edt3dsq(pasta::BitVector const &bv, const int64_t sx, const int64_t sy,
                const int64_t sz, const float wx, const float wy,
                const float wz, const bool black_border = false) noexcept {

  const int64_t sxy = sx * sy;
  const int64_t voxels = sz * sxy;

  std::unique_ptr<float[]> workspace =
      std::make_unique_for_overwrite<float[]>(sx * sy * sz);
    ...
    pool.detach_blocks(
        ...
        [&](const size_t start_idx, const size_t end_idx) noexcept {
          for (size_t i = start_idx; i < end_idx; i++) {
            size_t z = i / sy;
            size_t y = i % sy;
            squared_edt_1d_multi_seg(bv, workspace.get() + sx * y + sxy * z,
                                     sx * y + sxy * z, sx, wx, black_border);
          }
        });
    pool.wait();
  }
  if (!black_border) {
    tofinite(workspace.get(), voxels);
  }
    ...
    pool.detach_blocks(
        ...
        [&](const size_t start_idx, const size_t end_idx) noexcept {
          for (size_t i = start_idx; i < end_idx; i++) {
            ...
            _squared_edt_1d_parabolic(workspace.get() + offset + sx * y, sy - y,
                                      sx, wy, black_border || (y > 0),
                                      black_border);
          }
        });
    pool.wait();
    ...
    pool.detach_blocks(
        ...
        [&](const size_t start_idx, const size_t end_idx) noexcept {
          for (size_t i = start_idx; i < end_idx; i++) {
            ...
            _squared_edt_1d_parabolic(workspace.get() + offset + sxy * z,
                                      sz - z, sxy, wz, black_border || (z > 0),
                                      black_border);
          }
        });
    pool.wait();
  ...
  if (!black_border) {
    toinfinite(workspace.get(), voxels);
  }
...
inline std::unique_ptr<float[]>
binary_edtsq(pasta::BitVector const &bv, const int sx, const int sy,
             const int sz, const float wx, const float wy, const float wz,
             const bool black_border = false) noexcept {

  return _binary_edt3dsq(bv, sx, sy, sz, wx, wy, wz, black_border);
}
} // namespace edt