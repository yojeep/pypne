...
template <class T>
using stvecs = std::vector<std::vector<T>>;
using ststr = std::string;
using isstr = std::istringstream;

inline int readInt(isstr &in)
{
	int n = 0;
	in >> n;
	return n;
}
inline bool readBoolOr(std::string st, std::istream &in)
{
	in >> st;
	return st[0] == 'T' || st[0] == 't' || st[0] == 'Y' || st[0] == 'y' || st[0] == '1';
}
...
class InputFile
{ //! InputFile is a general input file reader, with some flexibility to chose the keyword endings etc
public:
		...
		: informative(inform), multiline_(multiline)
	{
		...
		if (fnam.empty())
		{
			data_.push_back({string("end"), string()});
			return;
		} //  initialize empty InputFile on demand

		read(fnam);

		for_i(data_) if ((data_[i].first == "include" || data_[i].first == "append") && !data_[i].second.empty())
		{
		...
		if (init)
		{
			...
			if (!wdir.empty() && wdir != "PWD" && wdir != "pwd")
			{
				if (wdir == "inputDir")
				{
					...
					if (sl < fnam.size())
						wdir = fnam.substr(0, sl);
				}
				std::cout << "Changing working directory: " << wdir << ",  " << _TRY_(chdir(wdir)) << std::endl;
			}
	...
	{ /// \return readnext,  true: keep reading same key, if returned sr not empty
		...
		for (;;)
		{
			...
			case ',':
				sr += '\t';
				break; //! comma is treated as white space
			...
			case '{':
				if (sr.size() && !noeq)
				{
					sf.sungetc();
					return true;
				} // try again with empty noeq=true (or empty sr to skip {})
				else if (noeq)
				{
					cr = '\t';
					do
			...
			case '<':
				...
				break; // primitive: filter out  xml tags
	...
	int read(std::istream &in, string fnam = "")
	{
		...
		if (fnam.size())
			fileName_ = fnam;
		if (data_.size() && data_.back().first == "end")
			data_.pop_back();
		...
		while (in.good())
		{
...

			{
...
#else
					bufr.find_first_of(":=", bgn + 1);
				if (endKy == string::npos)
					endKy = std::min(bufr.find_first_of(" \t", bgn + 1), lst);
#endif
				...
				if (bgn != string::npos)
					kydata = bufr.substr(bgn, lst - bgn);
			}
			while (readnext)
			{
	...
	{ // call this after setting name and/or prefix
		...
		if (lookup("name", name_) || lookup("TITLE", name_) || lookup("title", name_))
			name_ = prf + name_;
		else if (prf.size())
			name_ = prf;
		else
		{
			...
			if (prf.empty())
			{
				if (!lookup("ElementDataFile", prf))
					prf = fileName();
			}
			if (prf.size() > 7 && prf.substr(prf.size() - 3, 3) == ".gz")
				prf = prf.substr(0, prf.size() - 3); // sync bname
			size_t dl = prf.find_last_of(".");
			if (dl < prf.size())
				prf.erase(dl);
			size_t sl = prf.find_last_of("\\/");
			if (sl < prf.size())
				prf = prf.substr(sl + 1);
			name_ = prf;
		}
...
#define _debugInfo_(pref_) \
	if (debugLevel)        \
	std::cout << pref_ + key + ":" + data_[i].second << std::endl
#else
#define _debugInfo_(pref_)
#endif
	...
	void echoKeywords(std::ostream &out = std::cout) const
	{
		...
		if (out.tellp() == 0)
		{
			...
			out << "{/"
				   "/ -*- C -*- "
				<< outputName() << " input keywords:\n\n";
		}
		for_i(data_)
		{
			out << " " << data_[i].first << ":\t";
			if (data_[i].second.find_first_of("[:;(\"\'{") != string::npos)
				out << "{ " << data_[i].second << " }" << el << "\n";
			else
				out << data_[i].second << el << "\n";
		}
		if (el == '\n')
	...
	{ /// not recommended: cannot be refactored
		for_i(data_) if (data_[i].first == key)
		{
			_debugInfo_("[] ");
	...
	void renameKeys(string key, string newkey)
	{
		for_i(data_) if (data_[i].first == key)
		{
			_debugInfo_("Renaming ");
			data_[i].first = newkey;
		}
	}

	const string &kwrd(const string &key, int importance = 0) const
	{ //! get
		for_i(data_) if (data_[i].first == key)
		{
			_debugInfo_("Reading ");
	...
	bool giv(const string &key, isstr &iss, int importance = 0) const
	{ //! give me
		iss.clear();
		for_i(data_) if (data_[i].first == key)
		{
			_debugInfo_("Reading ");
			iss.str(data_[i].second);
			return true;
		}
		Assert(importance < 1, key, "missing keyword", importance > 1);
		return false;
	...
	template <class T>
	bool giv(const string &key, T &var, int importance = 0) const
	{ //! give me
		isstr iss;
		if (giv(key, iss, importance))
		{
			iss >> var;
			return true;
		}
	...
	template <class T>
	bool lookup(const string &key, T &var, int importance = 0) const { return giv(key, var); } /// similar to openfoam
	template <class T>
	T getOr(const string &key, T var) const
	{
		giv(key, var);
		return var;
	}
	...
	void checkEndOfData(isstr &iss, const string &key, const string message = "", bool severe = true) const
	{
		...
		char c;
		Assert(!iss.get(c), key, "Too much data", severe);
	}
...
public:
	...
	thread_local static std::ofstream dbgFile; /// output stream to write .dbg  file for debugging

	mstream(std::string fnam, unsigned char po = PRTF | STDO)
		: outps(po)
	{
		if (!fnam.empty() && outps & PRTF)
			prt_.open(fnam);
		if (!prt_)
			outps &= ~PRTF;
	};
