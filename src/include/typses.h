...
public:
	...
	var3 &operator^=(const var3 &v)
	{
		double r = y * v.z - z * v.y, s = z * v.x - x * v.z;
		z = x * v.y - y * v.x;
		x = r;
		y = s;
		return *this;
	...
	var3 operator/(const double t) const
	{
		double f = 1. / t;
		return var3(x * f, y * f, z * f);
	}
...
template <class T>
var3<T> rotateAroundLine(var3<T> b, double gamma, var3<T> n, var3<T> a)
{
...
template <class T>
var3<T> rotateAroundVec(const var3<T> b, double gamma, var3<T> n)
{
...
public:
	...
	var2(int r)
	{
		a = r;
		b = r;
	} // use this to zero-initialize //ERROR REMOVE ME
	var2(const T *d)
	{
		a = d[0];
		b = d[1];
	...
	var2 operator/(const double t) const
	{
		double f = 1. / t;
		return (var2(a * f, b * f));
	}
...
template <class T>
var3<T> operator*(double t, const var3<T> &v) { return var3<T>(t * v.x, t * v.y, t * v.z); }
inline dbl3 operator*(const int3 &v, const dbl3 &d) { return dbl3(v.x * d.x, v.y * d.y, v.z * d.z); } // boost int to T
template <class T>
T mag(const var3<T> &v) { return std::sqrt(v.x * v.x + v.y * v.y + v.z * v.z); }
template <class T>
T sum(const var3<T> &v) { return (v.x + v.y + v.z); }
template <class T>
double magSqr(const var3<T> &v) { return (v.x * v.x + v.y * v.y + v.z * v.z); }
template <class T>
var3<T> norm(const var3<T> &v) { return v / (mag(v) + 1e-300); }

//. component-wise max and min
template <class T>
var3<T> max(const var3<T> &a, const var3<T> &b) { return var3<T>(std::max(a.x, b.x), std::max(a.y, b.y), std::max(a.z, b.z)); }
template <class T>
var3<T> min(const var3<T> &a, const var3<T> &b) { return var3<T>(std::min(a.x, b.x), std::min(a.y, b.y), std::min(a.z, b.z)); }

template <class T>
T toScalar(const T &v) { return v; } // NOW IMPLICITly CONVERTED
template <class T>
T toScalar(const var3<T> &v) { return mag(v); }
...
public:
	...
	void reset(T *dd, int n)
	{
		d = dd;
		dn = d + n;
	};
	void reset(const piece &vs)
	{
		d = &vs[0];
		dn = d + vs.size();
	}; //! note data hold by piece are not const unless piece is const itself
	void reset(std::vector<T> &vs)
	{
		d = &vs[0];
		dn = d + vs.size();
	};
	...
	piece &fill(const T &v)
	{
		std::fill(d, dn, v);
		return *this;
	}
	piece &operator=(const piece &v)
	{
		ensure(size() == v.size(), "in operator =, piece sizes should be the same");
		std::copy(v.d, v.dn, d);
		return *this;
	}
	piece &operator+=(const piece &v)
...
public:
	...
	Vars(Vars &&v) noexcept : piece<T>(v.d, v.size())
	{
		v.d = 0;
		v.dn = 0;
	}
	...
	{
		if (d)
			delete[] d; /*(std::cout<<'~').flush();*/
	};

	Vars(Vars &v, bool move) : piece<T>(v.d, v.size())
	{
		v.d = 0;
		v.dn = 0;
		assert(move);
	} // same as above but enforced by move
	...
	void eat(Vars &v)
	{
		...
		dn = v.dn;
		v.d = 0;
		v.dn = 0; /*(std::cout<<'$').flush();*/
	};
	void operator=(const piece<T> &v)
	{
		{
			if (d)
				delete[] d;
	...
	void operator=(const std::vector<T> &v)
	{
		...
		d = new T[v.size()];
		std::copy(&v[0], &*v.end(), d);
		dn = d + v.size();
	};
	void operator=(const std::initializer_list<T> &v)
	{
		{
			if (d)
				delete[] d;
	...
	Vars &operator+=(const piece<T> &v)
	{
		this->piece<T>::operator+=(v);
		return *this;
	}
	Vars &operator-=(const piece<T> &v)
	{
	...
	void pbak(T &vj)
	{ // inefficient, use std::vector instead for dyn_array
		if (d)
		{
			T *od = d;
			d = new T[dn + 1 - d];
	...
	void pbak(const piece<T> vs)
	{
		...
		{
			T *od = d;
			d = new T[dn + vs.size() - d];
			std::copy(od, dn, d);
			dn = d + dn - od + vs.size();
			std::copy(vs.d, vs.dn, dn - vs.size());
			delete[] od;
		}
		else
		{
			d = new T[vs.size()];
			std::copy(vs.d, vs.dn, d);
			dn = d + vs.size();
		}
	}
};
...
template <class T>
using vars = Vars<T>; // or std::vector<T>;
...
template <size_t N>
using Dbl_ = std::array<double, N>;

template <class T>
Vars<T> operator-(const piece<T> &dis)
{
	Vars<T> rt(dis);
	for (auto &a : rt)
	{
...
template <class T>
Vars<T> operator+(const piece<T> &dis, const piece<T> &v)
{
	Vars<T> rt(dis);
	return rt += v;
}
...
Vars<U> operator*(const T dis, const piece<U> &v)
{
	Vars<U> rt(v);
	return rt *= dis;
}
template <class T>
Vars<var3<T>> operator*(const piece<T> &dis, const piece<var3<T>> &v)
{
	Vars<var3<T>> rt(v);
	return rt *= dis;
}
...
template <class T, class U>
Vars<T> operator/(const piece<T> &dis, const piece<U> &v)
{
	Vars<T> rt(dis);
	return rt /= v;
}
template <class T>
Vars<T> operator+(const piece<T> &dis, T t)
{
	Vars<T> rt(dis);
	rt += t;
	return rt;
...
template <class T>
Vars<T> operator*(const piece<T> &dis, double t)
{
	Vars<T> rt(dis);
	rt *= t;
	return rt;
...
template <class T>
Vars<T> operator*(const piece<T> &dis, int t)
{
	Vars<T> rt(dis);
	rt *= t;
	return rt;
...
template <class T>
Vars<T> operator/(double t, const piece<T> &dis)
{
	Vars<T> rt(dis);
	for (auto &a : rt)
	{
...
template <class T>
Vars<T> mag(const piece<var3<T>> &dis)
{
	Vars<T> rt(dis.size());
	for_(dis, i) { rt[i] = mag(dis[i]); }
	return rt;
}

template <class T>
Vars<T> operator&(const piece<var3<T>> &dis, const piece<var3<T>> &v)
{
	Vars<T> rt(dis.size());
	for_i(rt) { rt[i] = dis[i] & v[i]; }
	return rt;
}
...
public:
	...
	varsORv() : dfult(0), Xa(1), Xp(0), transf([](double a)
											   { return a; }),
				ax(1), px(0) {}
	varsORv(const T &val) : dfult(val), Xa(1), Xp(0), transf([](double a)
															 { return a; }),
							ax(1), px(0) {}
	varsORv(size_t siz, const T &val) : Vars<T>(siz, val), dfult(val), Xa(1), Xp(0), transf([](double a)
																							{ return a; }),
										ax(1), px(0) {}
	varsORv(const Vars<T> &vs) : Vars<T>(vs), dfult(vs.back()), Xa(1), Xp(0), transf([](double a)
																					 { return a; }),
								 ax(1), px(0) {}
	...
	varsORv(const T *dd, int nn) : Vars<T>(dd, nn), dfult(dd[nn - 1]), Xa(1), Xp(0), transf([](double a)
																							{ return a; }),
								   ax(1), px(0) {}
	varsORv(const T *dd, const T *de) : Vars<T>(dd, de), dfult(*(de - 1)), Xa(1), Xp(0), transf([](double a)
																								{ return a; }),
										ax(1), px(0) {}
	varsORv(const T *dd, const T *de, T (*fn)(T)) : Vars<T>(dd, de, fn), Xa(1), Xp(0), transf([](double a)
																							  { return a; }),
													ax(1), px(0) {}
	~varsORv() {};
	...
	void rescaledata(T d0, T del, T (*fn)(T))
	{
		Xp = d0;
		Xa = del;
		T *dd = d - 1;
	...
	void rescaledata(T d0, T del, T (*fn)(T), T p0, T a0)
	{
		Xp = d0;
		Xa = del;
		px = fn((p0 - Xp) / Xa);
		ax = fn((p0 + a0 - Xp) / Xa) - px;
		T *dd = d - 1;
		while (++dd < dn)
			*dd = (fn((*dd - Xp) / Xa) - px) / ax;
	}
...
{
	double px = fn((p0 - d0) / del);
	double ax = fn((p0 + a0 - d0) / del) - px;
	return (fn((val - d0) / del) - px) / ax;
}

template <class T>
Vars<T> operator-(T v, Vars<T> vs)
{
	Vars<T> rt(vs);
	for (auto &a : rt)
	{
...
template <class C>
int len(C vec) { return vec.size(); } // casts size to int

template <class T>
double sumdbl(const piece<T> &ps)
{
	double sm = 1e-300;
	for (auto a : ps)
	{
...
template <class T>
double sumdbl(const piece<T> &ps, const piece<T> &ws)
{
	double sm = 1e-300;
	const T *p = ps.d - 1, *w = ws.d - 1;
	while (++p < ps.dn)
	{
		sm += *w * *(++p);
	}
...
T sumq(piece<T> ps)
{
	...
	return sm;
} //! quick sum, assumes non-empty vec

template <class T>
T sum(const piece<T> &ps)
{
	T sm = (T() * 0.);
	const T *p = ps.d - 1;
	while (++p < ps.dn)
...
template <class T, class U>
T sum(const piece<T> &ps, const piece<U> &ws)
{
	T sm = (T() * 0.);
	T *p = ps.d - 1;
	U *w = ws.d - 1;
...
vars<T> sum(piece<piece<T>> ps)
{
	...
	return sm;
} // does not work for size zero

template <class T>
T sumSqrs(const piece<T> &ps)
{
	T sm = (T() * 0.);
	const T *p = ps.d - 1;
	while (++p < ps.dn)
...
template <class T, class U>
T sumSqrs(const piece<T> &ps, const piece<U> &ws)
{ // TODO remove refs
	T sm = (T() * 0.);
	const T *p = ps.d - 1;
	const U *w = ws.d - 1;
...
template <class T>
double sumdblSqrs(const piece<T> &ps)
{
	double sm = 0.;
	const T *p = ps.d - 1;
	while (++p < ps.dn)
...
template <class T>
T avg(const piece<T> &dis) { return sum(dis) / dis.size(); }
template <class T>
T min(const piece<T> &pis) { return *std::min_element(pis.d, pis.dn); }
template <class T>
T max(const piece<T> &pis) { return *std::max_element(pis.d, pis.dn); }
template <class T, class TFunc>
void transform(piece<T> &pis, TFunc func)
{
	for (T &dr : pis)
		dr = func(dr);
}
template <class T>
void transform(T *d, const T *dn, T (*func)(const T &))
{
	--d;
	while (++d < dn)
		*d = func(*d);
...
template <class T>
std::vector<T> &operator*=(std::vector<T> &vs, double scale)
{
	for (T &v : vs)
		v *= scale;
	return vs;
...
class Table : public TableIO<T, std::vector, C2>
{
public:
	...
	Table &apnd(const TableIO<T, std::vector, C2> &tbl)
	{
		for_i(tbl.vss_)
		{
			vss_.push_back(tbl.vss_[i]);
			if (hds_.size() && tbl.hds_.size())
				hds_.push_back(tbl.hds_[i]);
		}
		return *this;
	}
...
std::ostream &operator<<(std::ostream &out, const TableIO<T, C1, C2> &tbl)
{
	...
	{
		for_(tbl.hds_, j) { out << std::setw(12) << std::left << tbl.hds_[j] << tbl.sep_; }
		out << '\n';
	}
	for_(tbl.vss_[0], i)
	{
		for_(tbl.vss_, j) { out << std::setw(12) << std::left << tbl.vss_[j][i] << tbl.sep_; }
		out << '\n';
	}
	out << '\n';
	return out;
...
template <class T, template <class...> class C2>
std::istream &operator>>(std::istream &ins, Table<T, C2> &tbl)
{ // inefficient!!!
	ins >> std::ws;
	std::string row, item;
	if (std::is_arithmetic<T>::value && isalpha(ins.peek()))
	{
		...
		while (getline(ss, item, tbl.sep_))
			tbl.hds_.push_back(item);
	}
	...
	while (getline(ins, row))
	{
		...
		std::stringstream ss(row);
		while (getline(ss, item, tbl.sep_))
			Ro.push_back(strTo<T>(item));
		res.push_back(Ro);
	}
	if (res.size())
		tbl.vss_.resize(res[0].size());
	else
		return ins;
	for (size_t i = 0; i < tbl.vss_.size(); ++i)
	{
		tbl.vss_[i].resize(res.size());
		if (res[i].size() != tbl.vss_.size())
			std::cout << "Error table sizes don't match" << std::endl;
		for (size_t j = 0; j < tbl.vss_[i].size(); ++j)
			tbl.vss_[i][j] = res[j][i];
	}
	return ins;
}
...
template <class T>
std::istream &operator>>(std::istream &ins, std::vector<T> &vec)
{
	if (vec.size())
...
template <class T, class U>
std::ostream &operator<<(std::ostream &out, const std::map<T, U> &vec)
{
	for (auto v : vec)
	{
		out << v << '\n';
...
template <class T>
std::istream &operator>>(std::istream &ins, piece<T> vec)
{ // Warning relying on copy constructor not to deep copy
	for (; vec.d < vec.end(); ++vec.d)
		ins >> *vec.d;
...
std::istream &operator>>(std::istream &ins, std::array<T, N> &vec)
{
	for (auto &v : vec)
	{
		ins >> v;
...
inline void replaceInBetweenTo(std::string &str, const std::string &bgn, const std::string &end, const std::string &to)
{
	// no greedy macth
	size_t po = 0, po0 = 0;
	while ((po = str.find(bgn, po)) != std::string::npos)
	{
		po0 = po;
		po = po + bgn.length();
		while ((po = str.find(end, po)) != std::string::npos)
		{
			str.replace(po0, po - po0, to);
			po = po0 + to.length();
			break;
		}
	}
...
template <class T>
bool _1At(T n, int ibgn) { return n & (1 << ibgn); }
inline bool _1At(unsigned int n, int ibgn) { return n & (1 << ibgn); } // for debugger otherwise redundant
template <class T>
bool _1In(T n, int ibgn, int lnt) { return (n >> ibgn) & ((1 << lnt) - 1); }

// CLEAN UP PLEASE:

inline double roundec(double x, int d)
{
	double scale = std::pow(10, int(std::log10(x))) / std::pow(10, d);
	return std::round(x / scale) * scale;
} // round x to d significant digits

// template<class T> piece<T>   insidvars(piece<T>& vs) { return piece<T>(vs.d+1, vs.dn-1); }
template <class T>
Vars<T> diffVars(piece<T> vs)
{
	Vars<T> rt(vs);
	piece<T>(rt.d + 1, rt.dn) -= vs;
	rt[0] = rt[1];
	return rt;
}
template <class T>
Vars<T> movingAvg(piece<T> vs)
{ // apply rolling 3 moving average X <- ( X-- + X + X++)/3
	Vars<T> rt(vs.size());
	T *d = rt.d;
	*d = 0.5 * (vs[0] + vs[1]);
	for (T *o = vs.d + 2; o < vs.dn; ++o)
	{
		*(++d) = (1. / 3.) * (*(o - 2) + *(o - 1) + *o);
	}
	*(++d) = 0.5 * (*(vs.dn - 2) + *(vs.dn - 1));
	return rt;
} // TODO: use vs storage instead of o

template <class T>
T closer(T cv, T v1, T v2) { return (abs(cv - v1) < abs(cv - v2)) ? v1 : v2; }
...
template <class T>
Vars<T> biMovingAvg(piece<T> vs, int krnl, int bKrnl = 1)
{ // apply rolling 3 moving average X <- ( X-- + X + X++)/3
	if (len(vs) < 7)
		return vs;
	Vars<T> res(vs.size());
	T *rt = res.d - 1;
	const double pl = 1. / (1 + krnl);
	bKrnl = std::min(bKrnl, krnl);
	for (int i = 0; i < krnl; ++i)
	{
		int krp = std::max(i, bKrnl) + 1;
		*++rt = sumq(piece<T>(vs.d, krp) / krp);
	}
	vs.dn -= krnl;
	T *o = vs.d + krnl;
	for (; o < vs.dn; ++o)
	{
		*++rt = closer(*o, sumq(piece<T>(o - krnl, 1 + krnl)) * pl, sumq(piece<T>(o, 1 + krnl)) * pl);
	}
	--o;
	vs.dn += krnl;
	for (int i = krnl; i > 0; --i)
	{
		int krp = std::max(i, bKrnl) + 1;
		*++rt = sumq(piece<T>(vs.dn - krp, krp)) / krp;
	}
	return res;
}
...
T med_closer(vars<T> vs, T orig)
{
	...
	{
		const auto nth = vs.begin() + vs.size() / 2;
		std::nth_element(vs.begin(), nth, vs.end());
		return *nth;
	}
}
...
template <class T>
Vars<T> median(piece<T> vs, int krnl, int bKrnl = 1)
{ // apply rolling 3 moving average X <- ( X-- + X + X++)/3
	if (len(vs) < 7)
		return vs;
	Vars<T> res(vs.size());
	...
	bKrnl = std::min(bKrnl, krnl);
	for (int i = 0; i < krnl; ++i)
		*++rt = med_odd(vars<T>(vs.d, 2 * std::max(i, bKrnl) + 1));
	vs.dn -= krnl;
	T *o = vs.d + krnl;
	for (; o < vs.dn; ++o)
	{
		*++rt = med_odd(vars<T>(o - krnl, 1 + 2 * krnl));
	}
	--o;
	vs.dn += krnl; // vars -> piece ?
	for (int i = krnl; i > 0; --i)
	{
		int krn = std::max(i, bKrnl);
		*++rt = med_odd(vars<T>(vs.dn - 2 * krn - 1, 2 * krn + 1));
	}
	return res;
}
...
vars<dbls> distribution(const piece<T> xs, const piece<T> ws, int nBins = 64)
{
	...
	for (int i = 0; i < nBins; ++i)
		distrib[0][i] = minU + deltaU / 2 + i * deltaU;
