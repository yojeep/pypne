				...
				for (short i = s.s[ix].start + 1; i <= s.s[ix + 1].start; ++i)
				{
					...
					if (pID == rawValue)
					{
						...
						if (nDifferentID >= 3)
						{
							...
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k - 1][j - 1].vxl(i - 2)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
							neI = voxls.v_i(1, pijk);
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k - 1][j - 1].vxl(i)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
							neI = voxls.v_j(-1, pijk);
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k - 1][j - 2].vxl(i - 1)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
							neI = voxls.v_j(1, pijk);
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k - 1][j].vxl(i - 1)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
							neI = voxls.v_k(-1, pijk);
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k - 2][j - 1].vxl(i - 1)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
							neI = voxls.v_k(1, pijk);
							if (neI != pID && bgn <= neI && lst >= neI && cg.segs_[k][j - 1].vxl(i - 1)->R > R)
								++(neis.insert(pair<int, short>(neI, 0)).first->second);
