...
auto compute_PYPNEData_vector(blockNetwork &bn) {
  ...
  for (int pid = 2; pid < nNodes;
       ++pid) //. 0th and first are inlet and outlet elements
  {
  ...
  // for (std::map<int, int>::const_iterator bi = por.contacts.begin(); bi !=
  // por.contacts.end(); ++bi) {
...
auto compute_PYPNEData_string(blockNetwork &bn) {
  ...
  { /// ### 生成 _link1.dat 的内容到字符串
    ...
    for (int ti = 0; ti < int(throatIs.size()); ++ti) {
      const throatNE &tr = *throatIs[ti];
      ss << std::setw(6) << ti + 1 << " " << std::setw(6) << int(tr.e1 - 1)
         << " " << std::setw(6) << int(tr.e2 - 1) << " " << std::scientific
         << tr.radius() * dx << " " << std::scientific << t_shapeFacts[ti]
  ...
  { /// ### 生成 _link2.dat 的内容到字符串
    ...
    for (int ti = 0; ti < int(throatIs.size()); ++ti) {
      ...
      ss << std::setw(6) << ti + 1 << " " << std::setw(6) << int(tr.e1 - 1)
         << " " << std::setw(6) << int(tr.e2 - 1) << " " << std::scientific
         << t_lp1s[ti] * dx << " " << std::scientific << t_lp2s[ti] * dx << " "
         << std::scientific << t_ltrot[ti] * dx << " " << std::scientific
         << tr.volumn * dx * dx * dx << " " << 0. << "\n";
    }
    link2Content = ss.str();
  }
  ...
  { /// ### write  _node1.dat file 到字符串
    std::stringstream ss;
    // 写入头部（总节点数、nx, ny, nz）
    ss << std::setw(6) << (poreIs.size() - 2) << "  " << std::scientific
       << cg.nx * dx << "  " << std::scientific << cg.ny * dx << "  "
    ...
    for (int pid = 2; pid < int(poreIs.size()); ++pid) // 0和1是入口和出口
    {
      ...
      // 写入节点ID、坐标、contact数量
      ss << std::setw(6) << int(pid - 1) << " " << std::scientific
         << (por.mb->fi * dx + 0. * cg.X0[0]) << " " << std::scientific
         << (por.mb->fj * dx + 0. * cg.X0[1]) << " " << std::scientific
         << (por.mb->fk * dx + 0. * cg.X0[2]) << " " << std::setw(3)
      ...
      // 写入入口/出口标记（占位符）
      ss << "\t" << std::setw(5) << inlet << "\t" << std::setw(5) << outlet;
  ...
  { /// ### 生成 _node2.dat 的内容到字符串
    ...
    for (int pid = 2; pid < int(poreIs.size()); ++pid) {
      const poreNE &por = *poreIs[pid];
      ss << std::setw(6) << int(pid - 1) << " " << std::scientific
         << por.volumn * dx * dx * dx << " " << std::scientific
         << por.radius() * dx << " " << std::scientific << p_shape1s[pid] << " "
         << 0. << "\n";
    }
    node2Content = ss.str();
  }
