...
inline void usage(int detailed = 1)
{
	...
	if (detailed)
	{
		...
		cout << " For more information, please visit Imperial College pore-scale modelling website:\n"
				"https://www.imperial.ac.uk/earth-science/research/research-groups/pore-scale-modelling\n"
				"or contact Ali Q. Raeini: a.q.raeini@imperial.ac.uk"
			 << endl;
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (arg1 == "-g")
		return createSample_input_nextract(string(argc > 2 ? argv[2] : ""), arg1);
	srand(1001);
	try
	{
		auto fnam = arg1;
		if (hasExt(arg1, ".am") || hasExt(arg1, ".tif") || hasExt(arg1, ".gz") || hasExt(arg1, ".raw"))
			arg1 = "";
		inputDataNE cfg(arg1);
		if (arg1.empty())
			cfg.set("ElementDataFile", fnam);
...
int nextract(inputDataNE &cfg, bool verbose)
{
	...
	if (!cfg.getOr("overwrite", true) && ifstream(cfg.netName() + ".gnm"))
	{ // overwrite by default,  sync with WRITENETWORKG(), sync with tests
	...
	if (cfg.getOr("write_all", false))
	{ // use `write_all` as a rememberable alternative for all other visualization keywords
	...
	if (cfg.getOr("write_Statoil", false) || cfg.getOr("write_StatoilFormat", false))
		cfg.add("write_cnm", "true");
	...
	tim("write");
	if (cfg.getOr("write_radius", false))
		ballRadiiToVoxel(mpn).writeBin(cfg.name() + "_radius" + cfg.imgfrmt);

	tim("CreateVElem");
	mpn.CreateVElem(0);

	tim("write");
	if (cfg.getOr("write_elements", true))
		mpn.VElems.write(cfg.name() + "_VElems.mhd");
