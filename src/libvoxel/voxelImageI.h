...
#define forAlliii_(_vxls) OMPFor() for (size_t iii = 0; iii < _vxls.data_.size(); ++iii)

#define forAllvp_(_vxls) OMPFor() for (auto vp = _vxls.data_.data(); vp < &(*_vxls.data_.cend()); ++vp)
#define forAll_vp_(_vxls) OMPFor() for (auto vp = _vxls.data_.data() + _vxls.data_.size() - 1; vp > &(*_vxls.data_.cbegin()) - 1; --vp)

#define forAllkvp_(_vxls) OMPFor() for (int k = 0; k < (_vxls).nz(); ++k) for (auto vp = _vxls.data_.data() + k * _vxls.nij_, _ve_ = _vxls.data_.data() + (k + 1) * _vxls.nij_; vp < _ve_; ++vp)
#define forAllk_vp_(_vxls) OMPFor() for (int k = 0; k < (_vxls).nz(); ++k) for (auto vp = _vxls.data_.data() + (k + 1) * _vxls.nij_ - 1, _ve_ = _vxls.data_.data() + k * _vxls.nij_ - 1; vp > _ve_; --vp)
...
template <typename T>
Tint sum6Nei(const voxelImageT<T> &vf, const T *vp)
{
	return Tint(vf.v_i(-1, vp)) + vf.v_i(1, vp) + vf.v_j(-1, vp) + vf.v_j(1, vp) + vf.v_k(-1, vp) + vf.v_k(1, vp);
}

template <typename T>
int nSam6Nei(const voxelImageT<T> &vf, const T *vp)
{ /// \return number of adjacent voxels with same value, out of 6
	return Tint(vf.v_i(-1, vp) == *vp) + (vf.v_i(1, vp) == *vp) + (vf.v_j(-1, vp) == *vp) +
		   (vf.v_j(1, vp) == *vp) + (vf.v_k(-1, vp) == *vp) + (vf.v_k(1, vp) == *vp);
}

template <typename T>
int nSam6Nei2(const voxelImageT<T> &vf, const T *vp)
{ /// \return number of adjacent voxels with same value, out of 6, to add to nSameNei26
	return Tint(vf.v_i(-2, vp) == *vp) + (vf.v_i(2, vp) == *vp) + (vf.v_j(-2, vp) == *vp) +
		   (vf.v_j(2, vp) == *vp) + (vf.v_k(-2, vp) == *vp) + (vf.v_k(2, vp) == *vp);
}

template <typename T>
T maxNei(const voxelImageT<T> &image, int i, int j, int k, int r11, int r22)
{
...
TRes accumulate(const voxelField<T> &vf, TRes const &(*operatorFunc)(TRes const &, TRes const &), TRes result = 0)
{
	...
	OMPragma("omp parallel")
	{
		...
		OMPragma("omp for nowait") for (auto vp = vf.data_.begin(); vp < vf.data_.end(); ++vp)
		{
...
vars<dbls> vxlDist(const voxelFieldT &vf, int nsteps = 32, double minV = 3e38, double maxV = -3e38)
{
	...
	if (minV > 1e38)
		minV = accumulate(vf, dblfunc(std::min), 1e38);
	if (maxV < -1e38)
		maxV = accumulate(vf, dblfunc(std::max), -1e38);
	double spanV = (maxV - minV);
	double deltaV = spanV / nsteps * (1. + 1e-14);
	std::cout << "  vxlDist_range: [" << minV << " " << maxV << "]";

	for (int i = 0; i < nsteps; ++i)
		distrib[0][i] = minV + deltaV / 2 + i * deltaV;

	for (auto vv : vf.data_)
		distrib[1][std::min(std::max(0, int((vv - minV) / deltaV + 0.5)), nsteps)] += 1.;
...
template <typename T>
void voxelField<T>::reset(int3 nnn)
{
...
template <typename T>
void voxelField<T>::reset(int3 nnn, T value)
{
	this->data_.resize(0);
	nij_ = size_t(nnn.x) * nnn.y;
...
template <>
inline void voxelField<unsigned char>::readAscii(std::ifstream &in)
{
	int tmp;
	forAllvr_seq((*this))
	{
...
bool voxelField<T>::readAscii(std::string fnam)
{
	...
	if (!in)
	{
		std::cout << "\n\nError: can not open image file, " << fnam << std::endl
				  << std::endl;
		return false;
	}
...
template <typename T>
bool voxelImageT<T>::readAscii(std::string fnam)
{
	...
	{
		int3 nnn;
		in >> nnn.z >> nnn.x >> nnn.y; // ignore first lines
		dbl3 xmin, xmax;
		in >> xmin.x >> xmax.x >> xmin.y >> xmax.y >> xmin.z >> xmax.z;
...
// read order sensitive
template <typename T>
void voxelField<T>::readMicroCT(std::string fnam)
{
	std::cout << " micro-CT, reading " << fnam << std::endl;
	std::ifstream in(fnam);
	ensure(in, "can not open image file, " + fnam, -1);

	char tmpc;
	for (int i = 0; i < 8; ++i)
		in >> tmpc, std::cout << " " << tmpc; // ignore the first 8 characters (ascii 3uc)
	int3 nnn;
	dbl3 xmin, xmax;
...
inline std::string getAmiraDataType(const std::string &fnam)
{
	...
	while (true)
	{
		...
		if (hdr.fail())
		{
			std::cout << "Error reading " << fnam << ",  after " << tmp << std::endl;
			break;
		}
...
inline void getAmiraHeaderSize(const std::string &fnam, int3 &nnn, dbl3 &dx_, dbl3 &X0_, int &nSkipBytes, int &RLE)
{
	...
	while (true)
	{
		...
		if (hdr.fail())
		{
			std::cout << "Error reading " << fnam << ",  after " << tmpStr << std::endl;
			break;
		}
		std::string tmp;
		if (tmpStr == "define")
		{
			ss >> tmp >> nnn;
			if (tmp != "Lattice")
				std::cout << " Warning: define != Lattice n3, read: " << tmp << std::endl;
		}
		else if (tmpStr == "BoundingBox")
		{
			...
			int3 nn = nnn;
			for (int i : {0, 1, 2})
				nn[i] = std::max(nn[i] - 1, 1); // Why nnn-1? well it seems Avizo cannot even properly convert voxel size to bounding box, or something else is wrong in Abdollah way of doing things!!!
			dx_ = (dx_ - X0_) / nn;
		}
		else if (tmpStr == "@1")
			break;
	...
	nSkipBytes = hdr.tellg();
	++nSkipBytes; //++ is for '\n' after "@1"
}

template <typename T>
bool voxelField<T>::readBin(std::string fnam, int nSkipBytes)
{
	int3 nnn = size3();
	int RLEcompressed = 0;
	...
	if (hasExt(fnam, 3, ".gz"))
	{
		...
		if (std::ifstream(fnam).good())
		{
			...
			std::cout << "Error: could not be read " << fnam << std::endl;
#endif
...
bool voxelField<T>::readBin(std::string fnam, int iS, int iE, int jS, int jE, int kS, int kE, int nSkipBytes)
{
	if (hasExt(fnam, 4, ".tif"))
	{
		...
		if (vxls.nx() != iE - iS)
			std::cout << "Error in reading " << fnam << ", unexpected size: Nx=" << vxls.nx() << "  !=" << iE - iS << std::endl;
	...
	for (; k < kE; k++)
	{
		for (int j = jS; j < jE; ++j)
			if (in)
				in.read(reinterpret_cast<char *>(&((*this)(iS, j, k))), (iE - iS) * sizeof(T));
		if (!in)
			break;
	}
	if (!in)
...
template <typename T>
void voxelField<T>::writeBin(std::string fnam) const
{
	int3 nnn = size3();
	...
	{
#ifdef TIFLIB
		std::cout << "\n  writing tif file " << fnam << ";  size: " << nnn << " ";
		std::cout.flush();
		writeTif(*this, fnam);
		(std::cout << "  ").flush();
		return;
	...
	if (hasExt(fnam, 3, ".gz"))
	{
		std::cout << "\n  writing compressed file " << fnam << ";  size: " << nnn;
		std::cout.flush();
	...
	std::cout << "\n  writing binary file " << fnam << ";  size: " << nnn;
	std::cout.flush();
	auto mod = std::ios::out | std::ios::binary;
	if (hasExt(fnam, 3, ".am"))
	{ // in case write Header called before
		char cs[] = "xxx";
...
void voxelField<T>::writeBin(std::string fnam, int iS, int iE, int jS, int jE, int kS, int kE) const
{
	...
	{
#ifdef TIFLIB
		std::cout << "\n  writing tif file " << fnam << ";  i: " << iS << " " << iE << ",  j: " << jS << " " << jE << ",  k: " << kS << " " << kE;
		std::cout.flush();
	...
	if (hasExt(fnam, 3, ".gz"))
	{
		std::cout << "\n  writing compressed file " << fnam << ";  i: " << iS << " " << iE << ",  j: " << jS << " " << jE << ",  k: " << kS << " " << kE;
		std::cout.flush();
		gzofstream of(fnam.c_str());
	...
	std::cout << "\n  writing binary file " << fnam << ";  i: " << iS << " " << iE << ",  j: " << jS << " " << jE << ",  k: " << kS << " " << kE;
	std::cout.flush();
	auto mod = std::ios::out | std::ios::binary;
	if (hasExt(fnam, 3, ".am"))
	{
...
template <>
inline void voxelField<unsigned char>::writeAscii(std::string fnam, int iS, int iE, int jS, int jE, int kS, int kE) const
{
	std::cout << "\n  writing ascii file " << fnam << ";  ";
	std::cout.flush();
...
template <typename T>
void voxelField<T>::writeAscii(std::string fnam) const
{
	int3 imgsize = size3();
...
template <typename T>
void voxelField<T>::writeRotatedXZ(std::ofstream &of) const
{
...
template <typename T>
void voxelImageT<T>::writeHeader(std::string ofnam) const
{
...
void voxelField<T>::writeHeader(std::string fnam, int3 iS, int3 iE, dbl3 dx, dbl3 X0) const
{
	...
	{
		int islash = fnam.find_last_of("\\/");
		if (islash >= int(fnam.size()))
			islash = -1;
		std::string title = fnam.substr(islash + 1);
		if (hasExt(fnam, 4, ".mhd"))
			title = title.substr(0, title.size() - 4) + imgExt();
		else if (hasExt(fnam, 7, ".raw.gz"))
			fnam = fnam.substr(0, fnam.size() - 7) + ".mhd";
		else
			fnam = fnam.substr(0, fnam.find_last_of(".")) + ".mhd";

		std::string typeNmeVTK = "MET_UCHAR";
		if (typeid(T) == typeid(char))
			typeNmeVTK = "MET_CHAR";
		else if (typeid(T) == typeid(short))
			typeNmeVTK = "MET_SHORT";
		else if (typeid(T) == typeid(unsigned short))
			typeNmeVTK = "MET_USHORT";
		else if (typeid(T) == typeid(int))
			typeNmeVTK = "MET_INT";
		else if (typeid(T) == typeid(int))
			typeNmeVTK = "MET_UINT";
		else if (typeid(T) == typeid(float))
			typeNmeVTK = "MET_FLOAT";
		else if (typeid(T) == typeid(double))
			typeNmeVTK = "MET_DOUBLE";
		...
		of
			...
			<< "Offset =      " << X0 << std::endl
			<< "ElementDataFile = " << title << std::endl
			<< std::endl;
		if (dx.x >= 0.001)
			of << "Unit = " << 1 << std::endl;
...
{
	if (hasExt(fnam, 3, ".am"))
		writeHeader(fnam); // Warning: remember to append to this in writeBin
...
void voxelImageT<T>::write(std::string fnam) const
{
	...
	{
		if (hasExt(fnam, 4, ".mhd"))
			writeHeader(fnam = fnam.substr(0, fnam.size() - 4) + imgExt());
		else if (!hasExt(fnam, 4, ".tif"))
			writeHeader(fnam); //&& !hasExt(fnam,3,".am")!=0
...
void voxelImageT<T>::cropOld(int iS, int iE, int jS, int jE, int kS, int kE, int emptylyrs, T eLyrsValue)
{
	...
	if (emptylyrs)
		(std::cout << ", adding " << emptylyrs << " layers of " << double(eLyrsValue) << "  ").flush();
	ensure(iE < size3().x && jE < size3().y && kE < size3().z, "cropping outside bounds: " + _s(size3()), 2);

	X0_.x = X0_.x + (int(iS) - int(emptylyrs)) * dx_.x;
	X0_.y = X0_.y + (int(jS) - int(emptylyrs)) * dx_.y;
	X0_.z = X0_.z + (int(kS) - int(emptylyrs)) * dx_.z;
...
void voxelImageT<T>::cropD(int3 frm, int3 to, int emptylyrs, T eLyrsValue, bool verbose)
{
	...
	if (verbose)
		(std::cout << "  cropping from  [" << frm << " to " << to << ")  ").flush();
	else
		(std::cout << " D ").flush();
...
void voxelField<T>::setSlice(char dir, int ijk, T vv)
{
	if (dir == 'i')
		...
		std::cout << "Error: wrong dir " << dir << std::endl;
}
...
{
	forAllkji_(Values) (*this)(i + n1, j + n2, k + n3) = Values(i, j, k);
}
template <typename T>
...
void voxelImageT<T>::resetFrom(const voxelImageT<T2> &Values)
{
	...
	forAlliii_((*this)) (*this)(iii) = Values(iii);
}
template <typename T>
...
void voxelImageT<T>::growLabel(T vl)
{
	...
	forAllkji_1_(voxls)
	{
		if (voxls(i, j, k) == vl)
		{
			...
			if (voxls.v_i(-1, optr) != vl)
				(*this).v_i(-1, vptr) = vl;
			if (voxls.v_i(1, optr) != vl)
				(*this).v_i(1, vptr) = vl;
			if (voxls.v_j(-1, optr) != vl)
				(*this).v_j(-1, vptr) = vl;
			if (voxls.v_j(1, optr) != vl)
				(*this).v_j(1, vptr) = vl;
			if (voxls.v_k(-1, optr) != vl)
				(*this).v_k(-1, vptr) = vl;
			if (voxls.v_k(1, optr) != vl)
				(*this).v_k(1, vptr) = vl;
		}
...
template <typename T>
voxelImageT<T> resampleMean(const voxelImageT<T> &img, double nReSampleNotSafe)
{ //  TODO to be tested
...
{
public:
	bool operator()(std::pair<const T, short> &i1, std::pair<const T, short> i2) { return i1.second < i2.second; }
};

template <typename T>
voxelImageT<T> resliceZ(const voxelImageT<T> &img, double nReSampleNotSafe) //  TODO to be tested
{
	if (nReSampleNotSafe < 1e-64)
	{
		nReSampleNotSafe = img.dx().x / img.dx().z;
		(std::cout << " nResample: " << nReSampleNotSafe << ", ").flush();
	}
	voxelImageT<T> tmp;
	int3 nnn = img.size3();
	if (nReSampleNotSafe < .999)
	{
		...
		tmp.dxCh() = img.dx();
		tmp.dxCh().z *= nReS; // fixed
		tmp.X0Ch() = img.X0();
		tmp.X0Ch().z *= nReS; // fixed
		return tmp;
	}
	else if (nReSampleNotSafe > 1.001)
	{
	...
	else if (nReSampleNotSafe > 1.001)
	{
		...
		forAllkji_(tmp)
		{
			...
			forAllNei(0, nReS - 1)
			{
				...
				if (vj != pID)
					++(neis.insert(std::pair<T, short>(vj, 0)).first->second); // else ++nSames;
			}
...
{ // wrong X0
	...
	if (direction == 'z' || direction == 'Z')
	{
		...
		OMPFor() for (int k = 0; k < n3; ++k) for (int j = 0; j < n2; ++j)
		{
...
void voxelImageT<T>::PointMedian032(int nAdj0, int nAdj1, T lbl0, T lbl1)
{
	...
	forAllkji_1_sum_(voxls, reduction(+ : nChanges))
	{
		...
		if (lbl0 == vv || vv == lbl1)
		{
			...
			// neiSum-=voxls(i,j,k);
			if (neiSum0 >= nAdj0 && vv == lbl1)
			{
				vr = lbl0;
				++nChanges;
			}
			else if (neiSum1 >= nAdj1 && vv == lbl0)
			{
				vr = lbl1;
				++nChanges;
			}
...
void FaceMedGrowToFrom(voxelImageT<T> &vImg, T lblTo, T lblFrm, int ndif = 0)
{
	...
	forAllkji_1_sum_(voxls, reduction(+ : nChanges))
	{
		...
		if (vv == lblFrm)
		{
			...
			if (neiSumTo && neiSumTo > neiSum1 + ndif)
			{
				vImg(i, j, k) = lblTo;
				++nChanges;
			}
...
void voxelImageT<T>::shrinkPore()
{
	...
	OMPFor() for (int k = 1; k < voxls.nz() - 1; ++k)
	{
	...
	OMPFor() for (int j = 1; j < voxls.ny() - 1; ++j) for (int i = 1; i < voxls.nx() - 1; ++i)
	{
		int k = 0;
...
void voxelImageT<T>::mode(short minDif, bool verbose)
{
	...
	forAllkji_1_sum_(voxls, reduction(+ : nChanges))
	{
		...
		T
			vj = voxls.v_i(-1, vp);
		if (vj != pID)
			++(neis.insert(std::pair<T, short>(vj, 0)).first->second);
		else
			++nSames;
		vj = voxls.v_i(1, vp);
		if (vj != pID)
	...
	if (verbose)
		std::cout << "  mode_nChanges:" << nChanges << "; ";
}
...
long long modeNSames(voxelImageT<T> &vImage, const short nSameNei, bool verbose = false)
{
	...
	if (verbose)
		std::cout << "  modeNSames(" << nSameNei << ")_nChanges:" << nChanges << "; ";
	return nChanges;
}

template <typename T>
void voxelImageT<T>::growPore()
{ // optimized function, should be further optimized as it is frequently used
	...
	OMPFor() for (int k = 1; k < voxls.nz() - 1; ++k) for (int j = 1; j < voxls.ny() - 1; ++j)
	// for (int i=0; i<voxls.nx()-1; ++i)//
	{
		int i = 0;
...
{
	forAlliii_((*this)) (*this)(iii) = max((*this)(iii), data2(iii));
}

template <typename T>
void voxelImageT<T>::minEq(const voxelImageT &data2)
{
	forAlliii_((*this)) (*this)(iii) = min((*this)(iii), data2(iii));
}
...
void rescale(voxelImageT<T> &img, T theresholdMin, T theresholdMax)
{
	...
	OMPragma("omp parallel for reduction(min:vmin) reduction(max:vmax)")
		forAllcp(img)
	{
		vmin = std::min(*cp, vmin);
		vmax = std::max(*cp, vmin);
	}
	...
	for (int nShrink = 4; nShrink >= 0; nShrink--)
	{
		...
		for (int k = dataTmp.nz() * 3 / 4 - 2; k > dataTmp.nz() * 1 / 8 + 1; k--)
			for (int j = dataTmp.ny() * 3 / 4 - 2; j > dataTmp.ny() * 1 / 8 + 1; j--)
				...
				{
					if (dataTmp(i, j, k) == 0 &&
						!(dataTmp(i + 1, j + 1, k + 1) || dataTmp(i - 1, j + 1, k + 1) || dataTmp(i - 1, j - 1, k + 1) || dataTmp(i + 1, j - 1, k + 1) || dataTmp(i - 1, j - 1, k - 1) || dataTmp(i + 1, j - 1, k - 1) || dataTmp(i - 1, j + 1, k - 1) || dataTmp(i + 1, j + 1, k - 1) || dataTmp(i - 1, j, k) || dataTmp(i + 1, j, k) || dataTmp(i, j - 1, k) || dataTmp(i, j + 1, k) || dataTmp(i, j, k - 1) || dataTmp(i, j, k + 1)))
					{
						std::ofstream of(fnam);
						assert(of);
...
template <typename T>
void voxelImageT<T>::printInfo() const { ::printInfo(*this); }
...
void circleOut(voxelImageT<T> &vImage, int X0, int Y0, int R, char dir = 'z', T outVal = maxT(T))
{
	...
	if (dir == 'z')
	{
		forAllkji_(vImage) if ((i - X0) * (i - X0) + (j - Y0) * (j - Y0) > rlim)
			vImage(i, j, k) = outVal;
	}
	else if (dir == 'x')
	{
		...
		alert("Bad flip direction " + _s(dir), -1);
}
...
void maskWriteFraction(voxelImageT<T> &vImage, std::string maskname, std::string fnam, unsigned char maskvv, T minIelm, T maxIelm) //  TODO to be tested
{
	...
	if (of)
	{
		...
		for (T i = minIelm; i <= maxvv; ++i)
			of << double(nMasked[i]) / (nMasked[i] + nNotmsk[i] + 1e-38) << std::endl;
	}
}

template <typename T>
void mapToFrom(voxelImageT<T> &vImage, const voxelImageT<T> &vimg2, T vmin, T vmax, double scale = 0, double shift = 0.5)
{ // no interpolation
	...
	OMPFor(reduction(+ : count)) for (int k = N1.z; k < N2.z; ++k) for (int j = N1.y; j < N2.y; ++j) for (int i = N1.x; i < N2.x; ++i)
	{
		T &vv = vImage(i, j, k);
		if (vmin <= vv && vv <= vmax)
		{
...
template <typename T>
void mapToFrom(voxelImageT<T> &vImage, const voxelImageT<T> &vimg2)
{ // no interpolation
	int3 N2 = vImage.size3();
	size_t count = 0;
	int3 N1;
	...
	std::cout << " mapping bounds: " << Dn << " to " << N2 << std::endl;
	OMPFor() for (int k = N1.z; k < N2.z; ++k) for (int j = N1.y; j < N2.y; ++j) for (int i = N1.x; i < N2.x; ++i)
	{
...
operat(voxelImageT<T> &img1, char opr, const voxelImageT<T> &img2, std::stringstream &ins)
{
	...
	if (shift)
		std::cout << " " << opr << ":shift:" << shift << std::endl;
	else
		std::cout << " ";
	...
	{
	case '+':
		forAlliii_(img1) img1(iii) = std::max(Tint(minT(T)), std::min(Tint(img1(iii)) + (img2(iii) - shiftI), Tint(maxT(T))));
		break;
	case '-':
		forAlliii_(img1) img1(iii) = std::max(Tint(minT(T)), std::min(Tint(img1(iii)) - (img2(iii) - shiftI), Tint(maxT(T))));
		break; //! Analyse needs this
	case '&':
		forAlliii_(img1) { img1(iii) = img1(iii) & img2(iii); }
		break;
	case '|':
		forAlliii_(img1) { img1(iii) = img1(iii) | img2(iii); }
		break;
	...
	case '*':
		shift += 1.;
		forAlliii_(img1) { img1(iii) = std::min(Tint(double(img1(iii)) * img2(iii) * shift), Tint(maxT(T))); }
		break;
	case '/':
		shift += 1.;
		forAlliii_(img1) { img1(iii) = std::min(Tint(double(img1(iii)) / img2(iii) * shift), Tint(maxT(T))); }
	...
	case 'm': // map masked
	{
		...
		ensure(msk1Bgn <= msk1End);
		ensure(X0.x + img2.nx() <= img1.nx());
	...
	default:
		std::cout << " !!!operation " << opr << " voxelImage not supported!!! ";
		return 1;
	}
	return 0;
}
...
operat(voxelImageT<T> &vImg, char opr, std::string img2Nam, std::stringstream &ins)
{
	...
	if (img2Nam.empty())
	{
		...
		case '-':
			...
			break;
		default:
			std::cout << " !!!operation " << opr << " not supported!!! ";
		}
	}
	else
	{
		...
		{
			Tint ii;
			ii = strTo<Tint>(img2Nam);
			double dd;
			dd = strTo<double>(img2Nam);
			std::cout << "  image " << opr << "= " << dd << " ";
			switch (opr)
			{
			case '=':
				forAllvp_(vImg) { (*vp) = ii; }
				break;
			case '+':
				forAllvp_(vImg) { (*vp) += ii; }
				break;
			...
			case '*':
				if (dd > 1)
					forAllvp_(vImg) { (*vp) = std::min(Tint((*vp) * dd), Tint(maxT(T))); }
				else
				{
					forAllvp_(vImg) { (*vp) *= dd; }
				}
				break;
			case '/':
				forAllvp_(vImg) { (*vp) /= dd; }
				break;
			case 'b':
				forAllvp_(vImg) { (*vp) = std::max(ii, Tint(*vp)); }
				break;
			case 'e':
				forAllvp_(vImg) { (*vp) = std::min(ii, Tint(*vp)); }
				break;
			default:
				std::cout << " !!!operation " << opr << " " << img2Nam << " not supported!!! ";
			}
		}
		else
		{
...
operat(voxelImageT<T> &vImg, char opr, std::string img2Nam, std::stringstream &ins)
{
	...
	if (img2Nam.empty())
	{
		...
		case '-':
			...
			break;
		default:
			std::cout << "  Image  " << opr << "= !!!not supported!!! ";
		}
	}
	else
	{
		...
		else
		{
			...
			{
			case '+':
				forAlliii_(vImg) { vImg(iii) = vImg(iii) + img2(iii); }
				break;
			case '-':
