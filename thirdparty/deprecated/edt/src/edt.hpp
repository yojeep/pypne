 ...
 */
template <typename T>
float *_edt3dsq(T *labels, const int64_t sx, const int64_t sy, const int64_t sz,
  ...
  for (int64_t z = 0; z < sz; z++) {
    pool.enqueue([labels, sy, z, sx, sxy, wx, workspace, black_border]() {
      for (int64_t y = 0; y < sy; y++) {
        squared_edt_1d_multi_seg<T>((labels + sx * y + sxy * z),
  ...
  for (int64_t z = 0; z < sz; z++) {
    pool.enqueue([labels, sxy, z, workspace, sx, sy, wy, black_border]() {
      for (int64_t x = 0; x < sx; x++) {
        squared_edt_1d_parabolic_multi_seg<T>((labels + x + sxy * z),
                                              (workspace + x + sxy * z), sy, sx,
                                              wy, black_border);
      }
    });
  }
  ...
  for (int64_t y = 0; y < sy; y++) {
    pool.enqueue([labels, sx, y, workspace, sz, sxy, wz, black_border]() {
      for (int64_t x = 0; x < sx; x++) {
        squared_edt_1d_parabolic_multi_seg<T>((labels + x + sx * y),
                                              (workspace + x + sx * y), sz, sxy,
                                              wz, black_border);
      }
    });
  }
  ...
  for (z = 0; z < sz; z++) {
    for (x = 0; x < sx; x++) {
      ...
      pool.enqueue([sx, sy, y, workspace, wy, black_border, offset]() {
        _squared_edt_1d_parabolic((workspace + offset + sx * y), sy - y, sx, wy,
    ...
    for (x = 0; x < sx; x++) {
      ...
      pool.enqueue([sz, sxy, workspace, wz, black_border, offset]() {
        ...
        _squared_edt_1d_parabolic((workspace + offset + sxy * z), sz - z, sxy,
                                  wz, black_border || (z > 0), black_border);
      });
    }
  }
...
template <typename T>
float *_edt3dsq(bool *binaryimg, const int64_t sx, const int64_t sy,
                const int64_t sz, const float wx, const float wy,
                const float wz, const bool black_border = false,
                const int parallel = 1, float *workspace = NULL) {
...
// euclidean distance.
template <typename T>
float *_edt3d(T *input, const int64_t sx, const int64_t sy, const int64_t sz,
              const float wx, const float wy, const float wz,
              const bool black_border = false, const int parallel = 1,
              float *workspace = NULL) {
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt3d(T *input, const int64_t sx, const int64_t sy,
                     const int64_t sz, const float wx, const float wy,
                     const float wz, const bool black_border = false,
                     const int parallel = 1, float *workspace = NULL) {

  float *transform = _binary_edt3dsq<T>(input, sx, sy, sz, wx, wy, wz,
                                        black_border, parallel, workspace);
...
template <typename T>
float *_edt2dsq(T *input, const int64_t sx, const int64_t sy, const float wx,
                const float wy, const bool black_border = false,
                const int parallel = 1, float *workspace = NULL) {
  ...
  for (int64_t x = 0; x < sx; x++) {
    pool.enqueue([input, x, workspace, sy, sx, wy, black_border]() {
      squared_edt_1d_parabolic_multi_seg<T>((input + x), (workspace + x), sy,
                                            sx, wy, black_border);
    });
  }
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt2dsq(T *binaryimg, const int64_t sx, const int64_t sy,
  ...
  for (y = 0; y < sy; y++) {
    squared_edt_1d_multi_seg<T>((binaryimg + sx * y), (workspace + sx * y), sx,
                                1, wx, black_border);
  }
  ...
  for (x = 0; x < sx; x++) {
    pool.enqueue([workspace, x, sx, sy, wy, black_border]() {
      ...
      _squared_edt_1d_parabolic((workspace + x + y * sx), sy - y, sx, wy,
                                black_border || (y > 0), black_border);
    });
  }
...
// skipping multi-seg logic results in a large speedup
template <typename T>
float *_binary_edt2d(T *binaryimg, const int64_t sx, const int64_t sy,
                     const float wx, const float wy,
                     const bool black_border = false, const int parallel = 1,
                     float *output = NULL) {
...
// 2D version of _edt3dsq
template <typename T>
float *_edt2dsq(bool *binaryimg, const int64_t sx, const int64_t sy,
                const float wx, const float wy, const bool black_border = false,
                const int parallel = 1, float *output = NULL) {

  return _binary_edt2dsq(binaryimg, sx, sy, wx, wy, black_border, parallel,
                         output);
}

// returns euclidean distance instead of squared distance
template <typename T>
float *_edt2d(T *input, const int64_t sx, const int64_t sy, const float wx,
              const float wy, const bool black_border = false,
              const int parallel = 1, float *output = NULL) {

  float *transform =
      _edt2dsq<T>(input, sx, sy, wx, wy, black_border, parallel, output);
...
template <typename T>
float *edt(T *labels, const int sx, const float wx,
           const bool black_border = false) {
...
template <typename T>
float *edt(T *labels, const int sx, const int sy, const float wx,
           const float wy, const bool black_border = false,
           const int parallel = 1, float *output = NULL) {
...
template <typename T>
float *edt(T *labels, const int sx, const int sy, const int sz, const float wx,
           const float wy, const float wz, const bool black_border = false,
           const int parallel = 1, float *output = NULL) {

  return pyedt::_edt3d(labels, sx, sy, sz, wx, wy, wz, black_border, parallel,
                       output);
}

template <typename T>
float *binary_edt(T *labels, const int sx, const float wx,
                  const bool black_border = false) {
...
template <typename T>
float *binary_edt(T *labels, const int sx, const int sy, const float wx,
                  const float wy, const bool black_border = false,
                  const int parallel = 1, float *output = NULL) {
...
template <typename T>
float *binary_edt(T *labels, const int sx, const int sy, const int sz,
                  const float wx, const float wy, const float wz,
                  const bool black_border = false, const int parallel = 1,
                  float *output = NULL) {
...
template <typename T>
float *edtsq(T *labels, const int sx, const float wx,
             const bool black_border = false) {
...
template <typename T>
float *edtsq(T *labels, const int sx, const int sy, const float wx,
             const float wy, const bool black_border = false,
             const int parallel = 1, float *output = NULL) {
...
template <typename T>
float *edtsq(T *labels, const int sx, const int sy, const int sz,
             const float wx, const float wy, const float wz,
             const bool black_border = false, const int parallel = 1,
             float *output = NULL) {

  return pyedt::_edt3dsq(labels, sx, sy, sz, wx, wy, wz, black_border, parallel,
                         output);
}

template <typename T>
float *binary_edtsq(T *labels, const int sx, const float wx,
                    const bool black_border = false, const int parallel = 1) {
...
template <typename T>
float *binary_edtsq(T *labels, const int sx, const int sy, const float wx,
                    const float wy, const bool black_border = false,
...
float *binary_edtsq(T *labels, const int sx, const int sy, const int sz,
  ...
  return pyedt::_binary_edt3dsq(labels, sx, sy, sz, wx, wy, wz, black_border,
                                parallel, output);
}
