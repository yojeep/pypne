...
namespace MCTProcessing _begins_
	...
	template <typename T>
	bool ignore(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool fillHoles(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool info(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool selectPore(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool growPore(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	while (ins.good()) // loop while extraction from file is possible
	{
		if (alg == 'f')
		{
			...
			{
				std::cerr << "growing is only implemented for binary images: " << "selected voxel value to grow is " << vvalTogrow << ", which is not acceptable" << endl;
				return false;
			}
		}
		else
		{
			std::cerr << "selected growing algorithm: " << alg << " the only implemented algorithm is f which stands for faceGrowing" << endl;
			return false;
		}
...
template <typename T>
bool resampleMean(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool resampleMax(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool resliceZ(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool resampleMode(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool redirect(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool replaceRange(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool cropD(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	if (nLayers)
	{
		cout << "  + " << nLayers << " layers of " << value << " " << endl;
	}
...
template <typename T>
bool cropf(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool write(stringstream &ins, voxelImageT<T> &vImg)
{
	KeyHint("outputImageName.raw/mhd/tif/am/.raw.gz");
	string outName("dump.tif");
	ins >> outName;
	vImg.write(outName);
	(cout << ".").flush();
	return 0;
...
template <typename T>
bool write8bit(stringstream &ins, voxelImageT<T> &vImg)
{
	KeyHint("outputImageName_8bit.raw/mhd/tif/am/.raw.gz");
	string outName("dump.tif");
	ins >> outName;
	double minv = -0.5, maxv = 255.;
	ins >> minv >> maxv;
...
template <typename T>
bool read(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	if (fnam.size() > 4)
	{
		if ((nnn[2] && (hasExt(fnam, 7, ".raw.gz") || hasExt(fnam, 4, ".raw"))) || hasExt(fnam, 4, ".tif"))
		{
			...
			vImg.readFromHeader(fnam, processHdr);
	}
	return 0;
}

template <typename T>
bool readAtZ(stringstream &ins, voxelImageT<T> &vImg)
...
template <typename T>
bool medianFilter(stringstream &ins, voxelImageT<T> &vImg)
{
	KeyHint("nIterations");
	int nIterations(1);
	ins >> nIterations;
...
template <typename T>
bool modeFilter(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool medianX(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool FaceMedian06(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool PointMedian032(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	for (int i = 0; i < nItrs; ++i)
		vImg.PointMedian032(nAdj, nAdj, lbl0, lbl1);
...
template <typename T>
bool faceMedNgrowToFrom(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	cout << endl;
	for (int i = 0; i < nItrs; ++i)
		FaceMedGrowToFrom(vImg, T(lblTo), T(lblFrm), ndif);
	vImg.shrinkBox(2);
...
template <typename T>
bool delense032(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	for (int i = 0; i < nItrs; ++i)
		vImg.PointMedian032(25, nAdj1, lbl0, lbl1);
	FaceMedGrowToFrom(vImg, T(lbl1), T(lbl0), 1);
	FaceMedGrowToFrom(vImg, T(lbl0), T(lbl1), -1);
	for (int i = 0; i < 2 * nItrs; ++i)
	{
		vImg.PointMedian032(nAdj0, 25, lbl0, lbl1);
		FaceMedGrowToFrom(vImg, T(lbl0), T(lbl1), -1);
	}
...
template <typename T>
bool circleOut(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool maskWriteFraction(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool Offset(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool keepLargest0(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool growLabel(stringstream &ins, voxelImageT<T> &vImg)
{
...
template <typename T>
bool reset(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	while (param.size())
	{
		if (param == "maxNz")
		{
			ins >> maxNz;
			cout << "maxNz:" << maxNz << " ";
		} // stop reading after maxNz layers
		else if (param == "N")
		{
			int3 N{0, 0, 0};
			ins >> N;
		...
		else if (param[0] == 'N' && param[1] == 'd') // NdX or Nd0
		{
			...
			cout << "reset does not support " << param << endl;
		param = "";
		ins >> param;
	}
	(cout << ".").flush();
...
template <typename T>
bool operat(stringstream &ins, voxelImageT<T> &vImg)
{
...
bool mapFrom(stringstream &ins, voxelImageT<T> &vImg)
{
	...
	Tint minv(0), maxv(maxT(T));
	double scale = 0, shift(0.5 - T(0.5));
	string image2name;
	ins >> image2name >> minv >> maxv;
	ensure(maxv >= minv);
	cout << "\n{  mapping from image " << image2name << ", assigning to values originally in range: [" << minv << " " << maxv << "], by ";
	if (scale > 1e-16)
	{
		cout << shift << "+" << scale << "*";
	}
	cout << image2name << endl;
...
public:
	...
	int process(const InputFile &inp, voxelImageT<T> &img, string nam = "") const
	{ // nam is ignored here
		if (inp.data().size() > 2)
			std::cout << std::endl;
		for (const auto &ky : inp.data())
		{
			...
			else
			{
				if (ky.first != "end")
				{
					cout << "  stopped executing " + inp.fileName() + " before \"" + ky.first + "\"  :/ ";
					return -1;
				}
				break;
			}
...
{
	return voxelplugins<T>().process(ins, img, nam);
}

template <class InpT, typename First, typename... Rest>
int vxlProcess(const InpT &ins, voxelImageTBase *imgPtr, string nam)
{ //! detect type and run voxel plugins
...
string VxlKeysHelp(string keyname, string subkey)
{
	...
	if (keyname.size())
	{
		...
		if (paer != key_funs.end())
		{
			...
			{
				std::cerr << keyname << " KeyHelp not implemented:" << exc.what() << endl;
			}
			catch (...)
			{
				std::cerr << keyname << " KeyHelp not implemented:" << endl;
			}
			return ss.str();
		}
		else
	...
	else
	{
		...
		for (const auto &proc : keyfuns)
			if (proc.first.size() > 1)
			{
				stringstream ss("?");
				try
				{
				...
				{
					std::cerr << proc.first << " KeyHelp not implemented:" << exc.what() << endl;
				}
				catch (...)
				{
					std::cerr << proc.first << " KeyHelp not implemented:" << endl;
				}
...
void voxelImageT<T>::readFromHeader(istream &hdrFile, const string &hdrNam, int procesKeys)
{
	...
	if (hasExt(hdrNam, 4, ".mhd") || hasExt(hdrNam, 3, ".py"))
	{
		...
		while (true)
		{
			...
			if (ky == "ObjectType")
			{
				ss >> tmp;
				if (tmp != "Image")
					cout << "  Warning: ObjectType != Image :=" << tmp << endl;
			}
			else if (ky == "NDims")
			{
				ss >> tmp;
				if (tmp != "3")
					cout << "  Warning: NDims != 3 :=" << tmp << endl;
			}
			else if (ky == "ElementType")
			{
				ss >> tmp;
				if ((tmp != "MET_UCHAR") && (sizeof(T) == 1))
					cout << "  Warning: ElementType != MET_UCHAR :=" << tmp << endl;
			}
			else if (ky == "Offset")
			{
				ss >> vImg.X0_;
			...
			else if (ky == "ElementSize" || ky == "ElementSpacing")
			{
				ss >> vImg.dx_;
				cout << "  dX: " << vImg.dx_ << ",  ";
				dxread = true;
			...
			else if (ky == "ElementDataFile")
			{
				if (inputName.empty())
					ss >> inputName;
				size_t is = hdrNam.find_last_of("\\/");
				if (is < hdrNam.size() && inputName[0] != '/' && inputName[1] != ':')
					inputName = hdrNam.substr(0, is + 1) + inputName;
				cout << "  Img: " << inputName << ",	";
			}
			else if (ky == "BinaryData")
			{
				ss >> BinaryData;
			...
			else if (ky == "Unit")
			{
				ss >> unit_;
				autoUnit = false;
				cout << "  Unit, OneMeter: " << unit_ << endl;
			}
			else if (ky == "HeaderSize")
			{
				ss >> nSkipBytes;
				cout << "  Ski pHeaderSize: " << nSkipBytes << endl;
			}
			else if (ky == "OutputFormat" || ky == "DefaultImageFormat")
			{
				if (tmp == "=")
					ss >> tmp;
				cout << "  OutputFormat: " << tmp << ", suffix:" << imgExt(tmp) << "	" << endl;
			} ///. sets suffix+format
			else if (ky == "BinaryDataByteOrderMSB" || ky == "ElementByteOrderMSB")
			{
				ss >> flipSigByt;
			}
	...
	else if (hasExt(hdrNam, 7, ".raw.gz") || hasExt(hdrNam, 4, ".raw") || hasExt(hdrNam, 4, ".dat"))
	{ // detect size and voxel size from image name.
		string
	...
	else if (hasExt(hdrNam, 7, "_header"))
	{
		...
		char tmpc;
		for (int i = 0; i < 8; ++i)
			hdrFile >> tmpc, cout << tmpc; // ignore the first 8 characters (ascii 3uc)

		if (hasExt(hdrNam, 7, "_header"))
			inputName = hdrNam.substr(0, hdrNam.size() - 7);
		hdrFile >> nnn >> vImg.dx_ >> vImg.X0_;
		cout << "\n Nxyz: " << nnn << "    dX: " << vImg.dx_ << "   X0: " << vImg.X0_ << " um" << endl;
		if (!hdrFile)
		{
			cout << "   Incomplete/bad header name. Aborting" << endl;
			exit(-1);
		}
	}
	else
		alert("Unknown (header) file type: " + hdrNam, -1); // exit
	...
	if (abs(unit_ - 1.) > epsT(float))
		cout << "  unit= " << unit_ << " => dx= " << vImg.dx_ << ", X0= " << vImg.X0_ << endl;

	if (procesKeys)
		voxelplugins<T>().process(InputFile(hdrFile, hdrNam), vImg);
}
...
std::unique_ptr<voxelImageTBase> readImage(string hdrNam, int procesKeys)
{
	...
	if (hasExt(hdrNam, ".am"))
	{
		...
		if (vtype == "int")
			return make_unique<voxelImageT<int>>(hdrNam, 0);
		if (vtype == "short")
			return make_unique<voxelImageT<short>>(hdrNam, 0);
		if (vtype == "ushort")
			return make_unique<voxelImageT<unsigned short>>(hdrNam, 0);
		if (vtype == "byte")
			return make_unique<voxelImageT<unsigned char>>(hdrNam, 0);
...
#ifdef TIFLIB
	if (hasExt(hdrNam, ".tif"))
		return readTif(hdrNam);
#endif
	...
	else if (hasExt(hdrNam, 4, ".mhd"))
	{
		while (true)
		{
			...
			if (hdrFile.fail())
			{
				cout << "\n\n\nWarning: readImage, 'ElementType =' not set in " << hdrNam << endl;
				break;
			}
			if (ky == "ElementType")
			{
	...
	if (typ == "MET_UCHAR")
		return make_unique<voxelImageT<unsigned char>>(hdrNam, procesKeys);
	if (typ == "MET_CHAR")
		return make_unique<voxelImageT<char>>(hdrNam, procesKeys);
	if (typ == "MET_USHORT")
		return make_unique<voxelImageT<unsigned short>>(hdrNam, procesKeys);
	if (typ == "MET_SHORT")
		return make_unique<voxelImageT<short>>(hdrNam, procesKeys);
	if (typ == "MET_UINT")
		return make_unique<voxelImageT<unsigned int>>(hdrNam, procesKeys);
	if (typ == "MET_INT")
		return make_unique<voxelImageT<int>>(hdrNam, procesKeys);
	if (typ == "MET_FLOAT")
		return make_unique<voxelImageT<float>>(hdrNam, procesKeys);
	if (typ == "MET_DOUBLE")
		return make_unique<voxelImageT<double>>(hdrNam, procesKeys);
