...
template <typename T>
class voxelField //! 3D voxel data, on Cartesian uniform grids, with efficient access functions, and I/O into  .tif, .raw.gz .am, .raw, .dat file formats.
{
protected:
public:
	long long nij_;
...
public:
	...
	void readFromHeader(const std::string &headerName, int processKeys = 1)
	{
		if (!headerName.empty() && headerName != "NO_READ")
		{
			...
			if (hdr)
				this->readFromHeader(hdr, headerName, processKeys);
			else
				alert("cannot open header file, " + headerName);
		}
	}
	...
	double vv_mp5(double i, double j, double k) const
	{ /// linear interpolation
		...
		const T *
			vp = &(*this)(i0, j0, k0);
		const double v00 = *vp * ld + dd * this->v_i(1, vp);
		vp = this->p_j(1, vp);
		const double v10 = *vp * ld + dd * this->v_i(1, vp);
		vp = this->p_k(1, vp);
		const double v11 = *vp * ld + dd * this->v_i(1, vp);
		vp = this->p_j(-1, vp);
		const double v01 = *vp * ld + dd * this->v_i(1, vp);
		dd = j - j0;
		ld = 1. - dd;
		k -= k0;
...
int resetFromImageT(voxelImageT<T> &vImg, voxelImageTBase *imgPtr)
{ //! cast to specified vImg type
	if (auto img = dynamic_cast<voxelImageT<First> *>(imgPtr))
	{
		...
		return resetFromImageT<T, Rest...>(vImg, imgPtr);
	alert("Unknown image type in resetFromImageT");
	return -1;
}

template <typename T>
void readConvertFromHeader(voxelImageT<T> &vImg, std::string hdrNam, int procesKeys = 1)
{ //! read image and if needed convert its type
	...
	{
		vImg = std::move(*img);
	} // TODO ensure use swap
	else
		ensure(resetFromImageT(vImg, imgPtr) == 0, "can not convert image", -1);
}

template <class T>
voxelImageT<T> *vxlCast(voxelImageTBase *imgPtr) { return dynamic_cast<voxelImageT<T> *>(imgPtr); }
...
voxelImageT<T> copyOrReadImgT(std::string hdrNam)
{
	...
	{
		auto imgPtr = vxlCast<T>(static_cast<voxelImageTBase *>(ptr));
		ensure(imgPtr, "wrong image type", -1);
		return *imgPtr;
	}
	else
#endif
...
#define _dbgetOrReadImgT(_img, _hdrNam)                            \
	voxelImageT<T> *_imgPtr;                                       \
	std::unique_ptr<voxelImageTBase> _imgRead;                     \
	if (void *ptr = dbget(_STOR, _hdrNam, 0))                      \
	{                                                              \
		_imgPtr = vxlCast<T>(static_cast<voxelImageTBase *>(ptr)); \
	}                                                              \
	else                                                           \
	{                                                              \
		_imgRead = readImage(_hdrNam, 1);                          \
		_imgPtr = vxlCast<T>(_imgRead.get());                      \
	}                                                              \
