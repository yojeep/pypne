...
namespace boost
{
    namespace sort
    {
        namespace blk_detail
        {
            ...
            //-------------------------------------------------------------------------
            template <uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            merge_blocks<Block_size, Group_size, Iter_t, Compare>::merge_blocks(backbone_t &bkb, size_t pos_index1, size_t pos_index2,
                                                                                size_t pos_index3) : bk(bkb)
            {
                ...
                //  tail process
                //-------------------------------------------------------------------
                if (vpos2.back().pos() == (bk.nblock - 1) and bk.range_tail.first != bk.range_tail.last)
                {
            ...
            //-------------------------------------------------------------------------
            template <uint32_t Block_size, uint32_t Group_size, class Iter_t, class Compare>
            void merge_blocks<Block_size, Group_size, Iter_t, Compare>::tail_process(std::vector<block_pos> &vblkpos1,
                                                                                     std::vector<block_pos> &vblkpos2)
            {
            ...
            void merge_blocks<Block_size, Group_size, Iter_t, Compare>::cut_range(range_pos rng_input)
            {
                ...
                while (pos_ini < pos_last)
                {
                    ...
                    while (pos < pos_last and bk.index[pos - 1].side() == bk.index[pos].side())
                    {
                        ++pos;
                    }
                    if (pos < pos_last)
            ...
            void merge_blocks<Block_size, Group_size, Iter_t, Compare>::extract_ranges(range_pos range_input)
            {
                ...
                for (size_t posx = posx_ini + 1; posx <= range_input.last; ++posx)
                {
                    ...
                    if (not final)
                    {
                        ...
                        mergeable = (side_max != side_posx and is_mergeable(rng_max, rng_posx, bk.cmp));
                    }
                    if (bk.error)
                        return;
